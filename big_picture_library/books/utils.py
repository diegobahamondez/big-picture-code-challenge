...
def get_summary(book_data: dict):
    ...
    if isinstance(summary, dict): # from the records i've seen, the summary can come in the form of dict with "value" and "name" keys, or as a string directly
        summary = summary.get("value", None) # in some cases, it appears a description on the page, but the api doesn't return it. eg: 1526617161
...
def get_authors(book_data: dict):
    authors_records = book_data["data"].get("authors", [{}]) # in case there's more than one author,
    authors_list = [i["name"] for i in authors_records] # we make a list of them
    authors_string = ', '.join(authors_list) # and then join them as author1, author2 etc
